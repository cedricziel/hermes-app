...
- History is loaded over REST by `HermesChatRepository`. Sending and streaming go through the `ChatTransport` interface (`chat_transport.dart`), a sealed `ChatEvent` stream. The real implementation, `gateway/hermes_gateway_transport.dart`, speaks JSON-RPC over the dashboard's `/api/ws` websocket (`session.create` or `session.resume`, then `prompt.submit`, with replies as `message.delta` events). `gateway_rpc_client.dart` and `gateway_connection.dart` handle the socket. After a reply ends the transport keeps listening to its session, and `ChatTransport.followUps` carries the turns Hermes chains on its own (goal continuations, queued prompts). Session and prompt calls time out, and `checkConnection` (called when the app resumes) drops a socket the OS killed during sleep. A reply still running when its connection drops is not failed outright: the transport reopens the socket, resumes the session, and keeps following the same turn if the server reports it still running, so a lock or a dead socket does not lose a reply in progress.
- Domain messages (`chat_models.dart`) map to flyer messages in `chat_message_mapper.dart`. `chat_controller_sync.dart` applies before/after diffs to the `InMemoryChatController`. Custom message kinds and cards (tool calls, approval and clarify requests) live in `chat_message_kinds.dart` and `widgets/`. The design for agent input requests is in `openspec/changes/archive/2026-09-19-agent-input-requests/`. Text Hermes writes before a tool call, or checkpoints mid-turn with a `message.interim` event, is sealed into `ChatMessage.sealedProse` (`chat_reply.dart`) instead of being folded into the trailing `content`, so the mapper can render it where it was actually written and `message.complete` never overwrites it.
- The composer's model pill (`lib/src/models/widgets/composer_model_pill.dart`) picks a model and reasoning effort per chat from `GET /api/model/options` (`HermesModelsRepository`, loaded with each profile's threads, hidden when that fails). The pick lives on `ChatThread.modelChoice` and goes out with `ChatTransport.send`: as `session.create` overrides for a new chat, as session-scoped `config.set` (`model`, `reasoning`) on resume when it changed. Nothing is written to the profile's config.
...
**Helper models** (`lib/src/settings/helper_models_screen.dart`). The chat sidebar's "Helper models" row lists the chat profile's auxiliary task slots (`GET /api/model/auxiliary`) and opens the shared `showModelPicker` for one, with its "Use the profile's default" entry. The pick is saved once, when the picker closes, with `POST /api/model/set` (`scope: auxiliary`; the default entry sends `provider: auto`, empty model, which Hermes runs on the main model). Hermes may answer `confirm_required` for an expensive model; the screen asks and resends with `confirm_expensive_model`. Changes apply to new sessions only. Below the slots, a "Mixture of agents" section lists the default preset's advisors and aggregator (`GET /api/model/moa`, `MoaSetup`); a change sends the whole config back with `PUT /api/model/moa`, only that slot edited, and keeps the MoA rows closed until it answers. The section is left out when MoA cannot be read.
...
**Telemetry** (`lib/src/telemetry/`). OpenTelemetry to SignalDB through the `flutter_otel` package and its `dart_otel_*` siblings from pub.dev (source: `cedricziel/flutter-otel`, ours — file upstream PRs there rather than working around gaps locally). It is off unless `OTEL_EXPORTER_OTLP_ENDPOINT` is passed via `--dart-define`. When off, no SDK exists and no `traceparent` header is added. Telemetry code must never break the app (see `safely.dart`). Uncaught Flutter and async errors are logged with full message, stack trace and recent breadcrumbs via `flutter_otel`'s `installCrashReporting`/`BreadcrumbTrail`, wired up in `Telemetry.logUncaughtErrors`/`Telemetry.events`: unlike the package's privacy-preserving `installUncaughtErrorLogging` (type only, for an arbitrary OTLP endpoint), the crash reporter keeps full detail because the endpoint here is always the operator's own self-hosted SignalDB.
