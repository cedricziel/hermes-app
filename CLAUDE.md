...
- The composer is Hermes' own `ChatComposer` (`widgets/chat_composer.dart`), not flutter_chat_ui's `Composer`: a card with the field on top and attach, the model pill and send below it. `buildChatComposer` puts it in a `Positioned` that stays a direct child of the `Chat` stack and reports its height to the package's `ComposerHeightNotifier`, as the package's own composer does.
