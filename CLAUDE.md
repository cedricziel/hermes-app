...
**Helper models** (`lib/src/settings/helper_models_screen.dart`). The chat sidebar's "Helper models" row lists the chat profile's auxiliary task slots (`GET /api/model/auxiliary`) and opens the shared `showModelPicker` for one, with a "Same as main model" entry. The pick is saved once, when the picker closes, with `POST /api/model/set` (`scope: auxiliary`; "same as main" is `provider: auto`, empty model). Hermes may answer `confirm_required` for an expensive model; the screen asks and resends with `confirm_expensive_model`. Changes apply to new sessions only.
