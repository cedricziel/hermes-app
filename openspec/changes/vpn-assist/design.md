...
4. **Retry from `AuthController`,** subscribed to `changes` plus app resume (the app already has lifecycle observers, e.g. `AppShell`; the controller gets one more through `WidgetsBindingObserver` set up by the same owner that builds it). It calls `connect(saved, restoring: true)` only when `state == connectionError`, `lastFailure` is a network failure and no connect is in flight. `restoring: true` avoids flashing the connecting state. No "checking again" indicator is added, to keep the change small.
