...
- Add the camera usage description on iOS and the user-selected file entitlements on macOS.
...
- New dependencies `file_selector`, `image_picker`, `desktop_drop` and `pasteboard`, all maintained, with iOS, Android and desktop coverage between them. `file_selector` and `image_picker` are published by flutter.dev.
- Native: `NSCameraUsageDescription` in `ios/Runner/Info.plist`; `com.apple.security.files.user-selected.read-only` in both macOS entitlement files.
