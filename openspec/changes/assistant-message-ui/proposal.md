...
- **Code:** `lib/src/chat/`: a `ReasoningUpdated` transport event, `ChatMessage.reasoning`, the mapper's reasoning message and streaming flag, `ReasoningBlock` and `MessageActions` widgets, a retry path in the chat screen.
...
- Follow-up suggestion chips. The gateway sends none, and asking the model for them would cost a turn per reply.
