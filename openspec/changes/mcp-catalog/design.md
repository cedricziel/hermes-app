...
Confirmed against `packages/hermes_api` (`DefaultApi`): `listMcpCatalogApiMcpCatalogGet({profile})`, `installMcpCatalogEntryApiMcpCatalogInstallPost({required MCPCatalogInstall, profile})` with `MCPCatalogInstall(name, env, enable, profile)`, `getActionStatusApiActionsNameStatusGet({required name, lines})`, `authMcpServerApiMcpServersNameAuthPost({required name, profile})`, `mcpOauthFlowStatusApiMcpOauthFlowsFlowIdGet({required flowId})` and `cancelMcpOauthFlowApiMcpOauthFlowsFlowIdDelete({required flowId})`. The two flow routes take no `profile`; a flow id already identifies the profile it was started for. All six return `Response<Object>`, so the bodies are parsed by hand.
