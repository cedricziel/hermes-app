...
- **Picker generalisation.** `ModelPicker` and `showModelPicker` gain an optional `title` and an optional `defaultLabel`; with a label, an entry above the providers reports a `null` choice. The pill passes no label and never receives null.
