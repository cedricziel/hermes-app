...
- **Security and privacy:** this change lets a phone make the Hermes host run a program, so the review step is required for every command server, whichever way it arrives. Environment values and the bearer token are sent once in the request body and are never stored on the device, logged or put in telemetry, and their fields are obscured and cleared afterwards. The JSON editor shows the profile's real config, environment values and bearer tokens included (Hermes fills in the `${...}` references before it answers), so its text is never logged, persisted or sent to telemetry, and the screen says it holds secrets. Hermes' own check that rejects suspicious command configurations stays in force and its message is shown.
