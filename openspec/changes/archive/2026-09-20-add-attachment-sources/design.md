...
- **File picking: `file_picker`.** Already a dependency (Kanban attachments), covers all five platforms and resolves with our pins, so the app keeps one picker. The newest major release does not resolve against `win32 ^5`, which `flutter_secure_storage` and `package_info_plus` need, so the pinned `^11.0.3` stays. `file_selector` was considered first and dropped for the sake of a single picker.
...
Platforms: iOS (`NSCameraUsageDescription`), Android (none for the picker), macOS (`com.apple.security.files.user-selected.read-write` is already in `DebugProfile.entitlements` and `Release.entitlements`; nothing to add), Windows and Linux (none). watchOS is not affected. No invariant from the project context is touched.

## Risks / Trade-offs

- macOS sandbox handling of dropped files must be verified on a real build; if the drop scope is not enough, the user-selected entitlement already covers it.
